...
//  const { name, description, tools, code, demo, image, role } = project;
...
const SingleProject = ({ project }) => {
  ...
  return (
    <div className="group w-full h-fit flex flex-col items-center justify-center relative cursor-text overflow-hidden px-3 md:px-8 py-6 bg-[linear-gradient(90deg,#281e57_0%,#201435_100%)] shadow-2xl rounded-lg border border-[#1a1443]">
      ...
      <div className="relative w-full flex justify-center mb-4">
        <Image
          ...
          width={1080}
          height={720}
          className="w-80 h-64 transition-opacity duration-700 rounded-lg"
        />
      </div>

      {/* Project Name */}
      <h2 className="text-[#EFF3F4] font-semibold text-xl md:text-2xl text-center mb-1">{name}</h2>

      {/* Role */}
      <p className="text-orange-400 text-sm md:text-base mb-3">Role: {role}</p>

      {/* Description */}
      <p className="text-[#EFF3F4] text-sm md:text-base mb-3 text-center px-4">{description}</p>

      {/* Tools */}
      <div className="flex flex-wrap gap-2 justify-center mb-4">
        {tools.map((tool, idx) => (
          <span key={idx} className="bg-[#1a1443] text-[#16f2b3] px-2 py-1 rounded text-xs md:text-sm">
            {tool}
          </span>
        ))}
      </div>
